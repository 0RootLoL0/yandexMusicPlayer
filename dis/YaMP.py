...
class ExampleApp(QtWidgets.QMainWindow, dWindow.Ui_MainWindow):

	def __init__(self):
		super().__init__()
		self.setupUi(self)  # Это нужно для инициализации нашего дизайна
		self.SERVER = "127.0.0.1"  # адресс YaMPD
		self.PORT = 5591  # порт для YaMPD
		self.showIS = False  # переменная в которой храниться состояние окна ()
		self.pushButton_3.clicked.connect(self.nextYaMPC)
	...
	def action(self):
		if self.showIS:
			...
			#client.close()
			#self.label.setText(rnj["title"])
			#self.label_2.setText(rnj["artists"][0]["name"])
			self.show()
			self.showIS = True
